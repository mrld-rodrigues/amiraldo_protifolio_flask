...
def test_flask_app_import():
    ...
    try:
        print("\n🔍 Testando importação do flask_app.py...")
        import flask_app
        print(f"✅ flask_app.py importado com sucesso!")
        print(f"✅ Tipo da app: {type(flask_app.app)}")
        print(f"✅ Tipo da application: {type(flask_app.application)}")
        return True
    except Exception as e:
        print(f"❌ Erro ao importar flask_app.py: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_gunicorn_command():
    ...
    try:
        ...
        cmd = "python -c \"import wsgi; print('Gunicorn pode importar:', type(wsgi.app))\""
